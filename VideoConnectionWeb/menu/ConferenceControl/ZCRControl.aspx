        ...
        function ShowCHRYList() {//摄像头。麦克风。录制状态。退出房间
            ...
            for (var i = 0; i < VideoConferenceCHRY.length; i++) {
                ...
                if (OnLineList[VideoConferenceCHRY[i].ID] == 1) {
                    tbContent1 += '<td title="' + VideoConferenceCHRY[i].UserName + '"><div id="div' + VideoConferenceCHRY[i].ID + '" class="status-point" style=" background-color:#67C23A"></div>' + VideoConferenceCHRY[i].UserName + '</td>';
                }
                else {
                    tbContent1 += '<td title="' + VideoConferenceCHRY[i].UserName + '"><div id="div' + VideoConferenceCHRY[i].ID + '" class="status-point" style=" background-color:#00000040"></div>' + VideoConferenceCHRY[i].UserName + '</td>';
                }
                tbContent1 += '<td>';
