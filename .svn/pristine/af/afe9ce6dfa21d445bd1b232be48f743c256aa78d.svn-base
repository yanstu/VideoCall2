...
namespace VideoConnectionWeb.MyHub
{
    public class chatHub : Hub
    {
        ...
        public void SendHeartbeat(string channel, string mess)
        {
            if (StaticEntity.Redis_ZCR == null)
            {
                StaticEntity.Redis_ZCR = ConnectionMultiplexer.Connect(ConfigHelper.RedisIP + ":" + ConfigHelper.RedisPort + ",password=" + ConfigHelper.RedisPwd);

                StaticEntity.Redis_ZCR.GetSubscriber().Unsubscribe("VideoConference:" + channel);
                StaticEntity.Redis_ZCR.GetSubscriber().Subscribe("VideoConference:" + channel, (channel1, message) =>
                {
                    var hub = GlobalHost.ConnectionManager.GetHubContext<chatHub>();
                    hub.Clients.All.broadcastMessage(channel1.ToString(), message);

                });
            }
            if (StaticEntity.Redis_ZCR != null && StaticEntity.Redis_ZCR.IsConnected && !StaticEntity.Redis_ZCR.IsConnecting)
            {
                ISubscriber sub = StaticEntity.Redis_ZCR.GetSubscriber();
        ...
        public void ReConnect(string RoomId)
        {
            ...
            if (StaticEntity.Redis_ZCR != null && StaticEntity.Redis_ZCR.IsConnected && !StaticEntity.Redis_ZCR.IsConnecting)
            {
                StaticEntity.Redis_ZCR.GetSubscriber().Unsubscribe("VideoConference:" + RoomId);
                StaticEntity.Redis_ZCR.GetSubscriber().Subscribe("VideoConference:" + RoomId, (channel, message) =>
                {
                    var hub = GlobalHost.ConnectionManager.GetHubContext<chatHub>();
                    hub.Clients.All.broadcastMessage(channel.ToString(), message);

                    //str = StaticEntity.redis.GetDatabase().StringGet("mykey").ToString().Split('※');
                    //hub.Clients.All.broadcastMessage("mykey" + str[0], str[1]);
                });
            }
        }
        #endregion
